...
public class SphericalChaseCamera extends SphericalCamera {
    ...
    public SphericalChaseCamera(Vector3f playerPos) {
        super(new Vector3f(), playerPos);

        postionVector = new Vector3f(0.0f, -1.0f, -1.0f);
        distanceFromTarget = Constants.CAMERA_MIN_ZOOM;
        yawAboutTarget = 0.0;

        position.x = (postionVector.x * distanceFromTarget) + -playerPosition.getX();
        position.y = (postionVector.y * distanceFromTarget) + -playerPosition.getY();
        position.z = (postionVector.z * distanceFromTarget) + -playerPosition.getZ();
    ...
    public void moveCamera(float xChange, float yChange, int timeDelta) {
        ...
        Vector3f forwards = getForwardsVector();
        forwards.y = 0.0f;
        forwards.normalise();
        pitch = (float)Math.toDegrees((double)Vector3f.angle(postionVector, forwards));

        // Change pitch as camera zooms
        float p = (distanceFromTarget - Constants.CAMERA_MIN_ZOOM) /
                (Constants.CAMERA_MAX_ZOOM - Constants.CAMERA_MIN_ZOOM);
        pitch += 15.0f * p;
        p *= 12;

        position.x = (postionVector.x * (distanceFromTarget - p)) - playerPosition.getX();
        position.y = (postionVector.y * distanceFromTarget) - playerPosition.getY();
        position.z = (postionVector.z * (distanceFromTarget - p)) - playerPosition.getZ();
    }
    ...
    public Vector3f getRayToMousePosition() {
        ...
        return new Vector3f(rayViewPort.x, rayViewPort.y, rayViewPort.z);
    }

    @Override
    public void renderTargetBlock(int pId, int uId, Matrix4f perspectiveMatrix) {

        int[] blockPos = RayCaster.getIntercept(getPosition(), getRayToMousePosition(), Constants.MAX_PICK_DISTANCE);
